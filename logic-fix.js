(()=>{
'use strict';
if(window.__BUTIAN_LOGIC_FIX_RUNTIME_J__)return;
window.__BUTIAN_LOGIC_FIX_RUNTIME_J__=true;
// 2026-08-18j 自由文本宽容判定、动态反馈、事故报告回看与全流程稳定性修复
...
function openReportArchive(id){
  ...
  let modal=$('#nightReportArchive');
  if(!modal){
    modal=document.createElement('div');modal.id='nightReportArchive';modal.className='night-report-archive';modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');
    modal.innerHTML='<div class="night-report-sheet"><button type="button" class="night-report-close" aria-label="关闭">×</button><p class="night-report-kicker"></p><h2></h2><pre></pre><div class="night-report-actions"><button type="button" data-report-close>关闭报告</button></div></div>';
    document.body.append(modal);
    modal.querySelector('.night-report-close').onclick=()=>modal.remove();
    modal.querySelector('[data-report-close]').onclick=()=>modal.remove();
    modal.addEventListener('click',e=>{if(e.target===modal)modal.remove()});
  }
  setText(modal.querySelector('.night-report-kicker'),`${r.kind} · 已归档，可重复查看`);
  setText(modal.querySelector('h2'),r.title);
  setText(modal.querySelector('pre'),r.lines.join('\n'));
}
...
function apply(){
  ...
  patchQuestionOverlay();patchAudit();patchFinalChoice(s);patchDynamicEconomy();
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&inFlight){e.preventDefault();e.stopImmediatePropagation()}},true);
...
window.__BUTIAN_LOGIC_FIX__={version:'20260818j',readStored,confirmedEvents,confirmedInjuries,advanceTo,reportUnlocked,openReportArchive,NIGHT_REPORTS,localHitsFor,directionalPass,feedbackFor,questionIdFromOverlay};
})();
